...
The weighted sum (dot product) of a number of weighted sums is still a linear system and condenses down to a new weighted sum.
...
For a particular input and a particular output neuron there is a particular (switched) composition of weighted sums that may be condensed down into a single equivalent weighted sum.
You can look to see what that condensed weighted sum is looking at in the input or calculate some metrics like the angle between the input and the weight vector of the condensed weighted sum.
...
For a particular input and for a particular neuron in a ReLU neural network there is an equivalent single weighted sum.  And each weighed sum of each neuron is looking at different things in the input.  Actually paying attention to different aspects of the input.  Since weighted sums tend to be pattern detectors it is not surprising that artificial neural networks are sensitive to texture.
...
After storing 1 <pattern,response> item in the weighted sum the angle between the pattern and weight vector is zero and there is generally a strong amount of error correction from central limit theorem type averaging of the inputs.
...
The variance equation for linear combinations of random variables tells you the sort of preprocessing you need to do to turn the 
weighted sum into a general associative memory (eg. random projection, nonlinearity.)
